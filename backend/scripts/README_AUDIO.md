...
## Поиск аудиофайлов, отправленных без reply

Если ученик отправил голосовое сообщение без reply на сообщение бота, то:
- Submission создается (статус SENT)
- Но `answerFileId` не сохраняется (бот не обработал сообщение)

Для поиска таких аудиофайлов используйте скрипт `find-missing-audio.js`:

```bash
cd /var/www/tma_education/backend
export TELEGRAM_BOT_TOKEN="8580479721:AAF3Pn_h623BNYrAnJBJjD0LFpaYu13A-Mw"
node scripts/find-missing-audio.js YOUR_TELEGRAM_ID --users "Анна Фомина,Екатерина Кузьмина"
```

### Что делает скрипт:

1. Находит все submissions от указанных учеников:
   - `answerType = AUDIO` или `VIDEO`
   - `answerFileId = null` (файл не был сохранен)
   - `status = SENT`

2. Ищет соответствующие голосовые сообщения в истории Telegram через `getUpdates`

3. Сопоставляет сообщения по:
   - Telegram ID пользователя
   - Времени отправки (ближайшее по времени)

4. Отправляет найденные файлы куратору

5. Обновляет submissions с найденными `fileId`

### Ограничения:

⚠️ **Важно:** Telegram Bot API хранит обновления ограниченное время (обычно 24 часа). Если сообщения были отправлены давно, они могут быть недоступны через `getUpdates`.

Если скрипт не находит сообщения, попросите учеников отправить аудио заново с reply на сообщение бота.
